 ...
 *          - 新增 ETF成份证券现金替代类型(eOesEtfCashTypeT) 枚举类型定义
 * @version 0.17.6.2    2022/05/26
 *          - 调整 交易权限(eOesTradingPermissionT) 中部分权限的描述
 *              - 债券专业投资者 (OES_PERMIS_BOND_QUALIFIED_INVESTOR)
 *          - 调整 投资者适当性管理分类(eOesQualificationClassT) 中的描述
...
/** 批量委托接口可一次性原子发送的最大委托数量 (@note 超过该数量将自动拆分为多次提交, 可能会因为超出流量限制而导致部分委托被服务器端拒绝) */
#define OES_MAX_BATCH_ORDERS_SINGLE_COMMIT  (500)
...
/** 期权合约代码简称长度 */
#define OES_CONTRACT_SYMBOL_MAX_LEN         (56)
...
typedef enum _eOesEtfCashType {
    ...
    __OES_ETF_CASH_TYPE_USABLE_MIN          = 10,       /**< 可用替代资金最小值 */
    ...
    __OES_ETF_CASH_TYPE_MAX                             /**< 替代资金最大值 */
} eOesEtfCashTypeT;
 ...
 * ETF是否支持现金申赎标志
 */
typedef enum _eOesEtfAllCashFlag {
...
typedef enum _eOesTradingPermission {
    ...
    OES_PERMIS_BOND_QUALIFIED_INVESTOR      = (1ULL << 3),      /**< 债券专业投资者 */
...
typedef enum _eOesQualificationClass {
    ...
    OES_QUALIFICATION_QUALIFIED_INVESTOR    = 1,        /**< 专业投资者 */
