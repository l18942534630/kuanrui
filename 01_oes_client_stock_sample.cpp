 ...
 */
static int32
_OesClientMain_QueryColocationPeerCashAsset(
        Quant360::OesClientApi *pOesApi, const char *pCashAcctId) {
    return pOesApi->QueryColocationPeerCashAsset(pCashAcctId, 0);
}
...
main(void) {
    ...
    if (! pOesApi->LoadCfg(CONFIG_FILE_NAME)) {
        ...
        /*
         * 设置客户端的默认委托方式
         * @note 部分券商要求登录前填写委托方式
         */
        /*
        pOesApi->SetDefaultEntrustWay('K');
        */
    }
     ...
     */
    {
        ...
        /* 查询 两地交易时对端结点的资金信息 */
        _OesClientMain_QueryColocationPeerCashAsset(pOesApi, NULL);
