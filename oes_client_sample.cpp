...
/**
 * 设置客户端的默认委托方式 (需要在 Start 前调用才能生效)
 *
 ...
 */
BOOL
OesClientApi::SetDefaultEntrustWay(char entrustWay) {
    return OesApi_SetDefaultEntrustWay(entrustWay);
}
...
OesClientApi::Start(int32 *pLastClSeqNo, int64 lastRptSeqNum) {
    ...
    /* 启用内置的查询通道 (@note 将忽略配置文件中的参数, 固定启用内置的查询通道, 以供查询方法使用) */
    OesAsyncApi_SetBuiltinQueryable(_pAsyncContext, TRUE);

    /* 在启动前预创建并校验所有的连接 (@note 将忽略配置文件中的参数, 预创建和校验所有的连接, 以简化连接管理和异常处理) */
 ...
 */
int32
OesClientApi::QueryColocationPeerCashAsset(const char *pCashAcctId,
        int32 requestId) {
    return QueryColocationPeerCashAsset(_pDefaultOrdChannel, pCashAcctId,
            requestId);
}


/**
 * 查询两地交易时对端结点的资金资产信息 (基于指定的连接通道)
 *
 * @param   pAsyncChannel       指定的连接通道 (委托通道或回报通道均可)
 * @param   pCashAcctId         资金账号 (可以为空)
 *                              - 为空则返回当前连接对应的第一个客户的资金账户的对端结点资金资产信息
 ...
 */
int32
OesClientApi::QueryColocationPeerCashAsset(OesAsyncApiChannelT *pAsyncChannel,
        const char *pCashAcctId, int32 requestId) {
    OesCashAssetItemT       peerCashAssetItem = {NULLOBJ_OES_CASH_ASSET_ITEM};
    OesClientSpi            *pSpi = (OesClientSpi *) NULL;
    int32                   ret = 0;
    ...
    ret = OesAsyncApi_QueryColocationPeerCashAsset(pAsyncChannel, pCashAcctId,
            &peerCashAssetItem);
    if (__spk_unlikely(ret < 0)) {
        ...
        SLOG_ERROR("查询两地交易时对端结点的资金资产信息失败! ret[%d], error[%d - %s]",
                ret, OesApi_GetLastError(),
                OesApi_GetErrorMsg(OesApi_GetLastError()));
        return ret;
    }

    pSpi->OnQueryColocationPeerCashAsset(&peerCashAssetItem, requestId);
    return 1;
}
 ...
 */
void
OesClientSpi::OnQueryColocationPeerCashAsset(const OesCashAssetItemT *pCashAsset,
        int32 requestId) {
    _OesClientSpi_DefaultQryRspHandler(OESMSG_QRYMSG_COLOCATION_PEER_CASH,
            pCashAsset, NULL);
}
