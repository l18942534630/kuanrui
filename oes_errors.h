 ...
 *              - 1040, 数据已存在 (OESERR_EXIST)
 *              - 1291, 限制入金 (OESERR_DEPOSIT_LIMIT)
 *              - 1292, 限制出金 (OESERR_WITHDRAW_LIMIT)
 *              - 1293, 无可用现货系统节点 (OESERR_NO_AVAILABLE_STK_NODE)
 ...
 *          - 新增错误码定义
 *              - 1294, 无可用对端系统节点 (OESERR_NO_AVAILABLE_PEER_NODE)
 * @version 0.17.2.1    2021/08/09
 *          - 新增错误码定义
 *              - 1295, 对端限制入金 (OESERR_PEER_DEPOSIT_LIMIT)
 *              - 1296, 对端限制出金 (OESERR_PEER_WITHDRAW_LIMIT)
 ...
 *          - 删除错误码定义
 *              - 1276, 个股持仓比例超过限制 (OESERR_CUST_STK_POSITION_LIMIT)
 * @version 0.17.4.3    2021/10/31
 *          - 新增错误码定义
 *              - 1276, 限制交易上交所新债券平台 (OESERR_SSE_BOND_PLATFORM_LIMIT)
 * @version 0.17.4.6    2022/01/14
 *          - 新增错误码定义
 *              - 1449, 不能偿还非临近到期日负债 (OESERR_REPAY_NOT_EXPIRING_DEBT)
 *              - 1450, 可转出资产不足 (OESERR_TRSF_OUT_ASSERT_VALUE_NOT_ENOUGH)
 * @version 0.17.4.8    2022/05/13
 *          - 新增错误码定义
 *              - 1299, 股东账户没有交易公司债/企业债的权限 (OESERR_NO_CORPORATE_BOND_PERM)
 * @version 0.17.5      2021/09/19
 *          - 新增错误码定义
 ...
 * @version 0.17.6.1    2022/04/29
 *          - 新增错误码定义
 *              - 1043, 内部撤单 (OESERR_ORDER_DISCARDED)
 * @version 0.17.6.2    2022/05/26
 *          - 调整错误描述
 *              - 1260, 仅允许专业投资者交易该证券 (OESERR_QUALIFIED_INVESTOR_ONLY)
 *              - 1261, 仅允许专业机构投资者交易该证券 (OESERR_QUALIFIED_INSTITUTION_ONLY)
 *          - 废弃错误码“1252, 股东账户没有债券合格投资者的权限”, 改为使用错误码 “1260, 仅允许专业投资者交易该证券”
 *
 * @since   2015/07/30
 */
...
/** 1040, 数据已存在 */
#define OESERR_EXIST                            (&__ERRS_oesApiErrors[39])
/** 1041, 需要再次尝试 */
#define OESERR_TRY_AGAIN                        (&__ERRS_oesApiErrors[40])
/** 1042, 非法的委托方式 */
#define OESERR_ILLEGAL_ENTRUST_WAY              (&__ERRS_oesApiErrors[41])
/** 1043, 内部撤单 */
#define OESERR_ORDER_DISCARDED                  (&__ERRS_oesApiErrors[42])
/* -------------------------           */
 ...
 */
/** 1101, 登录柜台失败 */
#define OESERR_COUNTER_LOGON_FAILED             (&__ERRS_oesApiErrors[50])
/** 1102, 上报至柜台失败 */
#define OESERR_COUNTER_DLCR_FAILED              (&__ERRS_oesApiErrors[50 + 1])
/** 1103, 从柜台获取状态失败 */
#define OESERR_COUNTER_FETCH_FAILED             (&__ERRS_oesApiErrors[50 + 2])
/* -------------------------           */
...
/** 1252, 股东账户没有债券合格投资者的权限 @deprecated 已废弃, 为了兼容旧版本而保留 */
...
/** 1276, 限制交易上交所新债券平台 */
#define OESERR_SSE_BOND_PLATFORM_LIMIT          (&__ERRS_oesApiErrors[60 + 75])
/** 1277, 营业部委托流水号超出有效范围 */
...
/** 1293, 无可用现货系统节点 */
#define OESERR_NO_AVAILABLE_STK_NODE            (&__ERRS_oesApiErrors[60 + 92])
/** 1294, 无可用对端系统节点 */
#define OESERR_NO_AVAILABLE_PEER_NODE           (&__ERRS_oesApiErrors[60 + 93])
/** 1295, 对端限制入金 */
#define OESERR_PEER_DEPOSIT_LIMIT               (&__ERRS_oesApiErrors[60 + 94])
/** 1296, 对端限制出金 */
#define OESERR_PEER_WITHDRAW_LIMIT              (&__ERRS_oesApiErrors[60 + 95])
/** 1297, 禁止ETF申购 */
#define OESERR_ETF_CREATION_LIMIT               (&__ERRS_oesApiErrors[60 + 96])
/** 1298, 禁止ETF赎回 */
#define OESERR_ETF_REDEMPTION_LIMIT             (&__ERRS_oesApiErrors[60 + 97])
/** 1299, 股东账户没有交易公司债/企业债的权限 */
#define OESERR_NO_CORPORATE_BOND_PERM           (&__ERRS_oesApiErrors[60 + 98])
/* -------------------------           */
...
/** 1449, 不能偿还非临近到期日负债 */
#define OESERR_REPAY_NOT_EXPIRING_DEBT          (&__ERRS_oesApiErrors[200 + 48])
/** 1450, 可转出资产不足 */
#define OESERR_TRSF_OUT_ASSERT_VALUE_NOT_ENOUGH (&__ERRS_oesApiErrors[200 + 49])
/* -------------------------           */
...
/** 1541, 提取担保物后客户板块集中度超限 */
#define OESERR_CONCENTRATE_RATE_SECTOR_ON_TRANSFER_OUT                          \
                                                (&__ERRS_oesApiErrors[260 + 41])
/* -------------------------           */
