 ...
 *          - '客户端总览信息 (OesClientOverviewT)' 中
 *              - 新增 服务端是否支持与银行间银证转帐 (isSupportBankFundTrsf) 字段
 *          - '股东账户信息总览 (OesInvAcctOverviewT)' 中
...
typedef struct _OesInvAcctOverview {
    ...
    uint64              limits;                 /**< 证券账户权限限制 @see OES_LIMIT_xxx */
...
typedef struct _OesCustOverview {
    ...
    /** 客户姓名 */
    char                custName[OES_CUST_LONG_NAME_MAX_LEN];
    /** 备用字段 */
    char                __reserve[64];
} OesCustOverviewT;
...
typedef struct _OesClientOverview {
    ...
    uint8               isSupportBankFundTrsf;  /**< 是否支持与银行间银证转帐 */

    char                __reserve[124];         /**< 备用字段 */
...
typedef struct _OesBrokerParamsInfo {
    ...
    union {
        /** 允许客户端修改密码的开始时间 @deprecated 已废弃, 为了兼容旧版本而保留 */
        int32           changePwdLimitTime;
        /** 限制客户端修改密码的结束时间 (HHMMSSsss) */
        int32           forbidChangePwdEndTime;
    };
    /** 客户端密码允许的最小长度 */
    ...
    /** 服务端是否支持与银行间银证转帐 */
    uint8               isSupportBankFundTrsf;
    /** 按64位对齐的填充域 */
    uint8               __filler4;

    /** 限制客户端修改密码的开始时间 (HHMMSSsss) */
    int32               forbidChangePwdBeginTime;
