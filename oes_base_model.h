 ...
 *          - 调整 '成交基础信息(OesTrdBaseInfoT)' 和 '成交回报信息(OesTrdCnfmT)' 的结构体字段
 *              - 重命名 ETF赎回得到的替代资金是否当日可用(__isTrsfInCashAvailable) => ETF资金记录类型 (__etfCashType, 取值范围参考 eOesEtfCashTypeT)
 *          - '资金回报 (OesCashAssetReportT)'、'资金信息查询 (OesCashAssetItemT)' 中
 *              - 新增 日中委托流量费的累计发生金额 (ordTrafficFeeAmt) 字段
 *          - 证券账户基础信息 (OesInvAcctBaseInfoT) 中增加如下字段:
 *              - 风险警示板证券单日买入数量限制 (riskWarningSecurityBuyQtyLimit)
 * @version 0.17.6.3    2022/06/XX
 *          - 'ETF申赎产品基础信息 (OesEtfBaseInfoT)' 中
...
#define __OES_TRD_BASE_INFO_PKT                                         \
        ...
        /** 市场代码 @see eOesMarketIdT */ \
        uint8           mktId; \
        /** 买卖类型 (取值范围: 买/卖, 申购/赎回) @see eOesBuySellTypeT */ \
        uint8           trdSide; \
        /** 平台号 (OES内部使用) @see eOesPlatformIdT */ \
        uint8           __platformId; \
        ...
        uint8           __tgwGrpNo; \
        union { \
            /** ETF资金记录类型, 仅当成交类型为ETF资金记录时此字段有效 @see eOesEtfCashTypeT */ \
            uint8       __etfCashType; \
            /** ETF赎回得到的替代资金是否当日可用 (OES内部使用; 已废弃) */ \
...
#define __OES_INV_ACCT_BASE_INFO_PKT                                    \
        ...
        /** 风险警示板证券单日买入数量限制 */ \
        int32               riskWarningSecurityBuyQtyLimit; \
        \
        /** 预留的备用字段 */ \
        char                __INV_ACCT_BASE_reserve[28]
...
#define __OES_ETF_BASE_INFO_PKT                                         \
        ...
        uint8               isDisabled; \
        /** ETF是否支持现金申赎标志 @see eOesEtfAllCashFlagT */ \
        uint8               etfAllCashFlag; \
        \
        /** 成份证券数目 */ \
        int32               componentCnt; \
...
#define __OES_CASH_ASSET_BASE_INFO_PKT                                  \
        ...
        \
        /** 不可用金额(不可交易且不可提取, 目前仅包含上证ETF赎回所得的港市替代资金), 单位精确到元后四位, 即1元 = 10000 */ \
        int64               disableBal; \
...
#define __OES_CASH_ASSET_RPT_INFO_PKT                                   \
        ...
        int64               ordTrafficFeeAmt; \
        /** 预留的备用字段 */ \
        char                __CASH_ASSET_RPT_reserve[8]
