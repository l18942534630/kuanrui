...
/* 直接通过指定的参数初始化日志记录器 */
BOOL    SLog_InitLoggerDirect(
                const char *pLogSystemName,
                const char *pLogCategoryName,
                const char *pLogModeStr,
                const char *pMinLogLevelStr,
                const char *pLogFilePath,
                int32 maxFileLength,
                int32 maxBackupCount);

/* 根据配置结构初始化日志实例 */
BOOL    SLog_InitLoggerByCfgStruct(
                const SLogCfgItemT *pLogConfigs,
                int32 logCfgItemCount,
                const char *pLogSystemName);

/* 根据配置结构初始化指定日志分类的日志实例 (可指定待匹配的日志分类) */
BOOL    SLog_BuildLogInstanceWithCategories(
                const SLogCfgItemT *pLogConfigs,
                int32 logCfgItemCount,
                const SLogCategoryNameT *pLogCategoryList,
                int32 logCategoryCount,
                const char *pLogSystemName,
...
/* 设置线程级别的最小日志登记级别 @deprecated 已废弃, 不再支持单独设置线程级别的最小日志级别 */
...
void    _SLog_AssertSimplenessNA(BOOL EXPR, const char *fmt, ...);
