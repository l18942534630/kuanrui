...
/**
 * @file    01_oes_client_stock_sample.c
 *
 * OES API接口库的示例程序
 *
 ...
 */
static int32
_OesStkSample_QueryClientOverview(OesApiSessionInfoT *pSessionInfo) {
 ...
 */
static const char *
_OesStkSample_PrintStockStatus(const OesStockItemT *pStockItem,
 ...
 * @param   pSessionInfo    会话信息
 * @param   pMsgHead        消息头
 * @param   pMsgItem        消息体数据 @see OesStockItemT
 * @param   pCallbackParams 外部传入的参数
 * @return  大于等于0, 成功；小于0, 失败（错误号）
 */
static int32
_OesStkSample_OnQryStockCallback(OesApiSessionInfoT *pSessionInfo,
        SMsgHeadT *pMsgHead, void *pMsgItem, OesQryCursorT *pQryCursor,
        void *pCallbackParams) {
    OesStockItemT       *pStockItem = (OesStockItemT *) pMsgItem;

    /* 输出产品状态
     * '产品状态'值可能是多个状态位的组合, 调用函数_OesApiSample_PrintStockStatus输出状态名称
     * 默认通过 if (0) 禁用以下代码, 根据需要打开使用 */
    if (0) {
        char                buf[1024] = {0};
        _OesStkSample_PrintStockStatus(pStockItem, buf, sizeof(buf));
    }
 ...
 */
static inline int32
_OesStkSample_QueryStock(OesApiSessionInfoT *pQryChannel,
    ...
    ret = OesApi_QueryStock(pQryChannel, &qryFilter,
            _OesStkSample_OnQryStockCallback, NULL);
    if (__spk_unlikely(ret < 0)) {
 ...
 */
static int32
_OesStkSample_OnQryCashAssetCallback(OesApiSessionInfoT *pSessionInfo,
        SMsgHeadT *pMsgHead, void *pMsgItem, OesQryCursorT *pQryCursor,
        void *pCallbackParams) {
 ...
 */
static inline int32
_OesStkSample_QueryCashAsset(OesApiSessionInfoT *pQryChannel,
    ...
    ret = OesApi_QueryCashAsset(pQryChannel, &qryFilter,
            _OesStkSample_OnQryCashAssetCallback, NULL);
    if (__spk_unlikely(ret < 0)) {
 ...
 * @param   pSessionInfo    会话信息
 * @param   pMsgHead        消息头
 * @param   pMsgItem        消息体数据 @see OesStkHoldingItemT
 * @param   pCallbackParams 外部传入的参数
 * @return  大于等于0, 成功；小于0, 失败（错误号）
 */
static int32
_OesStkSample_OnQryStkHoldingCallback(OesApiSessionInfoT *pSessionInfo,
        SMsgHeadT *pMsgHead, void *pMsgItem, OesQryCursorT *pQryCursor,
        void *pCallbackParams) {
    OesStkHoldingItemT  *pHoldingItem = (OesStkHoldingItemT *) pMsgItem;
 ...
 */
static inline int32
_OesStkSample_QueryStkHolding(OesApiSessionInfoT *pQryChannel,
    ...
    ret = OesApi_QueryStkHolding(pQryChannel, &qryFilter,
            _OesStkSample_OnQryStkHoldingCallback, NULL);
    if (__spk_unlikely(ret < 0)) {
 ...
 * @param   pSessionInfo    会话信息
 * @param   pMsgHead        消息头
 * @param   pMsgItem        消息体数据 @see OesMarketStateItemT
 * @param   pCallbackParams 外部传入的参数
 * @return  大于等于0, 成功；小于0, 失败（错误号）
 */
static int32
_OesStkSample_OnQryMarketStateCallback(OesApiSessionInfoT *pSessionInfo,
        SMsgHeadT *pMsgHead, void *pMsgItem, OesQryCursorT *pQryCursor,
        void *pCallbackParams) {
    OesMarketStateItemT   *pMktStateItem = (OesMarketStateItemT *) pMsgItem;
 ...
 */
static inline int32
_OesStkSample_QueryMarketStatus(OesApiSessionInfoT *pQryChannel,
    ...
    ret = OesApi_QueryMarketState(pQryChannel, &qryFilter,
            _OesStkSample_OnQryMarketStateCallback, NULL);
    if (__spk_unlikely(ret < 0)) {
 ...
 */
static inline int32
_OesStkSample_OnTimeout(OesApiClientEnvT *pClientEnv) {
...
OesStkSample_ReportThreadMain(OesApiClientEnvT *pClientEnv) {
    ...
    while (! *pThreadTerminatedFlag) {
        ...
        ret = OesApi_WaitReportMsg(pRptChannel, THE_TIMEOUT_MS,
                _OesStkSample_HandleReportMsg, NULL);
        if (__spk_unlikely(ret < 0)) {
            if (__spk_likely(SPK_IS_NEG_ETIMEDOUT(ret))) {
                /* 执行超时检查 (检查会话是否已超时) */
                if (__spk_likely(_OesStkSample_OnTimeout(pClientEnv) == 0)) {
                    continue;
                }
...
OesStkSample_Main() {
...
#if ! (defined (__WINDOWS__) || defined (__MINGW__))
    {
        ...
        ret = pthread_create(&rptThreadId, NULL,
                (void* (*)(void *)) OesStkSample_ReportThreadMain,
                &cliEnv);
        if (ret != 0) {
...
#else
    {
        ...
        rptThreadId = CreateThread(NULL, 0,
                ...
                ((void *) OesStkSample_ReportThreadMain),
     ...
     */
    {
        ...
        _OesStkSample_QueryMarketStatus(&cliEnv.qryChannel, OES_EXCH_SZSE,
                OES_PLATFORM_CASH_AUCTION);

        /* 查询 客户端总览信息 */
        _OesStkSample_QueryClientOverview(&cliEnv.qryChannel);

        /* 查询 所有关联资金账户的资金信息 */
        _OesStkSample_QueryCashAsset(&cliEnv.qryChannel, NULL);

        /* 查询 指定资金账户的资金信息 */
        /* _OesStkSample_QueryCashAsset(&cliEnv.qryChannel, "指定资金账户"); */

        /* 查询 指定上证 600000 的产品信息 */
        _OesStkSample_QueryStock(&cliEnv.qryChannel, "600000",
                OES_MKT_ID_UNDEFINE, OES_SECURITY_TYPE_UNDEFINE,
                OES_SUB_SECURITY_TYPE_UNDEFINE);

        /* 查询 上海A股市场 的产品信息 */
        /* _OesStkSample_QueryStock(&cliEnv.qryChannel, NULL,
                OES_MKT_SH_ASHARE, OES_SECURITY_TYPE_UNDEFINE,
                OES_SUB_SECURITY_TYPE_UNDEFINE); */

        /* 查询 上证 600000 股票的持仓 */
        _OesStkSample_QueryStkHolding(&cliEnv.qryChannel, OES_MKT_SH_ASHARE,
                "600000");
        ...
        /* 查询 沪深两市 所有股票持仓 */
        _OesStkSample_QueryStkHolding(&cliEnv.qryChannel, OES_MKT_ID_UNDEFINE,
                NULL);
    }
     ...
     */
    {
         ...
         */
        _OesStkSample_SendOrderReq(&cliEnv.ordChannel, OES_MKT_SH_ASHARE,
         ...
         */
        _OesStkSample_SendOrderReq(&cliEnv.ordChannel, OES_MKT_SZ_ASHARE,
         ...
         */
        _OesStkSample_SendOrderReq(&cliEnv.ordChannel, OES_MKT_SH_ASHARE,
                "204001", NULL, OES_ORD_TYPE_LMT, OES_BS_TYPE_REVERSE_REPO,
                1000, 12350);
         ...
         */
        _OesStkSample_SendOrderReq(&cliEnv.ordChannel, OES_MKT_SZ_ASHARE,
                "131810", NULL, OES_ORD_TYPE_LMT, OES_BS_TYPE_REVERSE_REPO,
                10, 43210);
     ...
     */
    {
        ...
        _OesStkSample_SendOrderCancelReq(&cliEnv.ordChannel,
