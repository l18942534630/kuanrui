 ...
 * @version 0.17.4.8    2022/05/10
 *          - 调整 '行情数据类别 (eMdsSubStreamTypeT)' 枚举类型定义, 增加行情数据类别:
 *              - 深交所债券现券交易匹配成交大额逐笔行情 (MDS_SUB_STREAM_TYPE_BOND_BLOCK_TRADE, 仅适用于深交所逐笔行情)
 * @version 0.17.4.8_u1 2022/05/24
 *          - 调整 '上交所逐笔委托的订单类型 (eMdsL2SseOrderTypeT)' 枚举类型定义, 增加订单类型:
 *              - 产品状态订单 (MDS_L2_SSE_ORDER_TYPE_STATUS)
 * @version 0.17.6.1    2022/05/13
 *          - 调整 'Level-1快照行情 (MdsStockSnapshotBodyT)' 结构体定义, 增加如下深交所债券行情专用字段:
 ...
 *          - 调整 '上交所逐笔委托的订单类型 (eMdsL2SseOrderTypeT)' 枚举类型定义, 增加订单类型:
 *              - 产品状态订单 (MDS_L2_SSE_ORDER_TYPE_STATUS)
 * @version 0.17.6.3_u1 2022/06/28
 *          - 调整 '期权合约信息的静态数据 (MdsOptionStaticInfoT)' 结构体定义:
 *              - 期权合约名称字段 (securityName) 长度由40字节调整为56字节, 其长度宏定义
 *                由 MDS_MAX_SECURITY_NAME_LEN 替换为 MDS_MAX_CONTRACT_SYMBOL_LEN
 *
 * @since   2016/02/11
 */
...
/** 期权合约简称长度 */
#define MDS_MAX_CONTRACT_SYMBOL_LEN             (56)
 ...
 * 行情数据类别
 */
typedef enum _eMdsSubStreamType {
 ...
 */
typedef enum _eMdsMdStreamType_Legacy001512 {
    /*
     * Level-1 行情快照数据类型
     */
...
typedef struct _MdsTradingSessionStatusMsg {
    ...
    uint8               mdProductType;          /**< 行情产品类型 (股票/指数/期权) @see eMdsMdProductTypeT */
    ...
    uint16              __dataVersion;          /**< 行情数据的更新版本号 (当__isRepeated!=0时, 该值仅作为参考值) */
    uint16              __filler;               /**< 按64位对齐的填充域 */
...
typedef struct _MdsSecurityStatusMsg {
    uint8               exchId;                 /**< 交易所代码(沪/深) @see eMdsExchangeIdT */
    uint8               mdProductType;          /**< 行情产品类型 (股票(包含基金和债券)/期权) @see eMdsMdProductTypeT */
    int8                __isRepeated;           /**< 是否是重复的行情 (供内部使用, 小于0 表示数据倒流) */
    uint8               __origMdSource;         /**< 原始行情数据来源 @see eMdsMsgSourceT */
...
typedef struct _MdsMktDataSnapshotHead {
    ...
    int32               updateTime;             /**< 行情时间 (HHMMSSsss, 交易所时间) */

    int32               instrId;                /**< 证券代码 (转换为整数类型的证券代码) */
    int16               bodyLength;             /**< 实际数据长度 */
    union {
        uint8           bodyType;               /**< 快照数据对应的消息类型 @see eMdsMsgTypeT */
        uint8           mdStreamType;           /**< 快照数据对应的消息类型 @deprecated 已废弃, 为了兼容旧版本而暂时保留 */
    };
    uint8               subStreamType;          /**< 行情数据类别 @see eMdsSubStreamTypeT */
    uint16              __channelNo;            /**< 频道代码 (仅适用于深交所, 对于上交所快照该字段无意义, 取值范围[0..9999]) */
    uint16              __dataVersion;          /**< 行情数据的更新版本号 */
...
typedef struct _MdsStockSnapshotBody {
    ...
    int32               ClosePx;                /**< 今收盘价/期权收盘价 (适用于上交所行情和深交所债券现券交易产品) */
    ...
    union {
        ...
        struct {
            ...
            uint64      TotalLongPosition;      /**< 期权合约总持仓量 (适用于期权) */
        };

        /* 仅适用于债券产品的字段 (为支持新债券业务改造而增加的字段) */
        struct {
            /** 深交所债券加权平均价 (仅适用于深交所质押式回购及债券现券交易产品, 表示质押式回购成交量加权平均利率及债券现券交易成交量加权平均价. 价格单位精确到元后四位, 即: 1元=10000) */
            int32       BondWeightedAvgPx;
            /**
             ...
             */
            int32       BondAuctionTradePx;
            /** 深交所债券匹配成交的成交总量 (仅适用于深交所债券现券交易产品) */
            uint64      BondAuctionVolumeTraded;
        };
    };
...
typedef struct _MdsL2StockSnapshotBody {
    ...
    uint64              NumTrades;              /**< 成交笔数 */
    uint64              TotalVolumeTraded;      /**< 成交总量 (@note 上交所债券的数量单位为手) */
    int64               TotalValueTraded;       /**< 成交总金额 (金额单位精确到元后四位, 即: 1元=10000) */
    ...
    int32               LowPx;                  /**< 最低价 */
    int32               TradePx;                /**< 成交价 */
    int32               ClosePx;                /**< 今收盘价/期权收盘价 (加权平均收盘价, 适用于上交所行情和深交所债券现券交易产品) */
    ...
    union {
        ...
        /* 仅适用于债券产品的字段 (为支持新债券业务改造而增加的字段) */
        struct {
            /** 债券加权平均价 (适用于质押式回购及债券现券交易产品, 表示质押式回购成交量加权平均利率及债券现券交易成交量加权平均价. 价格单位精确到元后四位, 即: 1元=10000) */
            int32       BondWeightedAvgPx;
            /**
             * 深交所债券匹配成交的最近成交价 (仅适用于深交所债券现券交易产品. 价格单位精确到元后四位, 即: 1元=10000)
             * - @note 如果证券还没有产生匹配成交交易方式的成交, 则该字段为0
    ...
    union {
        ...
        /* 仅适用于深交所的债券现券交易相关字段 (为支持新债券业务改造而增加的字段) */
        struct {
            int64       BondAuctionValueTraded; /**< 深交所债券匹配成交的成交总金额 (仅适用于深交所债券现券交易产品. 金额单位精确到元后四位, 即: 1元=10000) */
        };
    };
...
typedef struct _MdsL2Trade {
    uint8               exchId;                 /**< 交易所代码(沪/深) @see eMdsExchangeIdT */
    uint8               mdProductType;          /**< 行情产品类型 (股票) @see eMdsMdProductTypeT */
    ...
    int32               instrId;                /**< 证券代码 (转换为整数类型的证券代码) */
    uint16              ChannelNo;              /**< 频道代码 [0..9999] */
    uint16              __reserve;              /**< 按64位对齐的保留字段 */
    /**
     * 深交所消息记录号/上交所成交序号 (从1开始, 按频道连续)
     * - 深交所为逐笔成交+逐笔委托统一编号
     * - 上交所新债券行情为逐笔成交+逐笔委托统一编号 (TickIndex)
     * - 上交所竞价行情(非债券行情)为逐笔成交独立编号 (TradeIndex)
     */
    int32               ApplSeqNum;
...
typedef struct _MdsL2Order {
     ...
     * - 深交所为逐笔成交+逐笔委托统一编号
     * - 上交所新债券行情为逐笔成交+逐笔委托统一编号 (TickIndex)
     * - 上交所竞价行情(非债券行情)为逐笔委托独立编号 (OrderIndex)
     */
    int32               ApplSeqNum;
     ...
     * - 上交所: '1'=买, '2'=卖
     *   - @note 对于上交所新债券行情的产品状态订单(OrderType='S'), Side 字段取值含义如下:
     *     - 'A': ADD – 产品未上市
     *     - 'S': START – 启动
     *     - 'O': OCALL – 开市集合竞价
     *     - 'T': TRADE – 连续自动撮合
     ...
     * - 上交所: 'A'=委托订单-增加(新订单), 'D'=委托订单-删除(撤单), 'S'=产品状态订单(仅适用于上交所新债券行情)
     */
    char                OrderType;
    /** 行情数据类别 @see eMdsSubStreamTypeT */
    uint8               subStreamType;
    ...
    /**
     * 委托数量
     * - @note 对于上交所竞价行情(非债券行情), 该字段的含义为剩余委托量或撤单数量:
...
typedef struct _MdsStockStaticInfo {
    ...
    char                securityId[MDS_MAX_INSTR_CODE_LEN];
    uint8               exchId;                 /**< 交易所代码 (沪/深) @see eMdsExchangeIdT */
    uint8               mdProductType;          /**< 行情产品类型 (股票/期权/指数) @see eMdsMdProductTypeT */
    ...
    uint8               pricingMethod;          /**< 计价方式 (仅适用于债券 @see eOesPricingMethodT) */
    uint8               __filler2[6];           /**< 按64位对齐的填充域 */
...
typedef struct _MdsOptionStaticInfo {
    ...
    /** 期权合约名称 (UTF-8 编码) */
    char                securityName[MDS_MAX_CONTRACT_SYMBOL_LEN];
    /** 标的证券代码 */
    char                underlyingSecurityId[MDS_MAX_INSTR_CODE_LEN];
    /** 按64位对齐的填充域 */
    uint8               __filler2[7];
