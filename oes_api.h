 ...
 * @version 0.17.4.3    2021/10/28
 *          - 优化出入金金额的精度处理, 自动将出入金金额的有效精度向下舍入分, 以方便前端使用 (例如: 金额 1.9999 将被自动转换为 1.9900)
 ...
 *                                - 致命错误 (FATAL)
 *                                - 屏蔽所有日志 (NONE)
 * @param   pLogFilePath        日志文件路径
 * @param   maxFileLength       日志文件最大长度
 *                              - 取值小于0, 表示无最大长度限制
 *                              - 取值等于0, 将使用默认值 (400M)
 *                              - 如果取值小于2048, 将以兆为单位计算, 否则以字节为单位计算, 最大文件长度为2GB
 * @param   maxBackupCount      日志文件最大备份数
 *                              - 轮换(ROLLING)模式下的最大保留的历史日志文件数量
 *                              - 取值小于0, 表示不保留备份文件
 *                              - 取值等于0, 将使用默认值 (3)
 * @return  TRUE 成功; FALSE 失败
 */
BOOL    OesApi_InitLoggerDirect(
                const char *pLogMode,
                const char *pMinLogLevel,
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL    OesApi_SetDefaultEntrustWay(
                char entrustWay);
 ...
 * @return  客户端默认的委托方式
 */
char    OesApi_GetDefaultEntrustWay();

/**
 * 设置客户端会话使用的委托方式 (仅对当前会话生效)
 *
 * @param   pSessionInfo        会话信息
 * @param   entrustWay          委托方式
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL    OesApi_SetAssignedEntrustWay(
                OesApiSessionInfoT *pSessionInfo,
                char entrustWay);

/**
 * 获取客户端会话使用的委托方式 (仅对当前会话生效)
 *
 * @return  客户端会话使用的委托方式
 */
char    OesApi_GetAssignedEntrustWay(
