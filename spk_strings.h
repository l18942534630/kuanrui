...
#include    <sutil/string/spk_safe_libc_strings.h>
...
SStr_StrCatChStr(char *pBuf, char ch, const char *pStr, int32 maxStrlen) {
    ...
    if (__spk_unlikely(! pPtr || maxStrlen == 0)) {
        return pBuf;
    }
 ...
 * - a     => a
 *
 * @param       pStr            待解析的目标字符串
 * @param       pDelimChars     分割符列表
 * @param[out]  pBuf            用于输出数据的缓存区指针
 * @param       bufSize         缓存区大小
 * @return      上级区段名称; NULL, 参数为空
 */
static __inline const char*
SStr_GetParentSection(const char *pStr, const char *pDelimChars, char *pBuf,
        int32 bufSize) {
    const char          *pPtr = pStr;
    const char          *pBase = pStr;

    if (__spk_unlikely(! pStr || ! pDelimChars || ! pBuf || bufSize <= 0)) {
        return (const char *) NULL;
    }

    if (pDelimChars[1] == '\0') {
        const char      delimChar = *pDelimChars;
        ...
        for (; *pPtr; pPtr++) {
            if (SStr_IsAnyChar(*pPtr, pDelimChars)) {
                pBase = pPtr;
            }
        }
    }

    return SStr_StrCopy(pBuf, pStr, SPK_MIN(bufSize - 1, pBase - pStr));
}
 ...
 * @param       pStr            待解析的目标字符串
 * @param       pDelimChars     分割符列表
 * @return      末尾区段名称; NULL, 参数为空
 */
static __inline const char*
SStr_GetLastSection(const char *pStr, const char *pDelimChars) {
    const char          *pPtr = pStr;
    const char          *pBase = pStr;

    if (__spk_unlikely(! pStr || ! pDelimChars)) {
        return (const char *) NULL;
    }
