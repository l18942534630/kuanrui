...
OesClientMySpi::OnCashAssetVariation(const OesCashAssetReportT *pCashAssetRpt) {
    fprintf(stdout, ">>> 收到资金变动回报: " \
            ...
            pCashAssetRpt->feeFrzAmt, pCashAssetRpt->ordTrafficFeeAmt,
            pCashAssetRpt->marginAmt, pCashAssetRpt->marginFrzAmt,
            pCashAssetRpt->totalInternalAllotAmt,
 ...
 */
void
OesClientMySpi::OnQueryColocationPeerCashAsset(const OesCashAssetItemT *pCashAsset,
        int32 requestId) {
    fprintf(stdout, ">>> 查询到对端节点的资金信息: " \
            "资金账户代码[%s], 客户代码[%s], 币种[%" __SPK_FMT_HH__ "u], " \
            "资金类型[%" __SPK_FMT_HH__ "u], 期初余额[%" __SPK_FMT_LL__ "d], " \
            "期初可用[%" __SPK_FMT_LL__ "d], 期初可取[%" __SPK_FMT_LL__ "d], " \
            "不可用[%" __SPK_FMT_LL__ "d], 累计存入[%" __SPK_FMT_LL__ "d], " \
...
OesClientMySpi::OnQueryCustInfo(const OesCustItemT *pCust,
    ...
    fprintf(stdout, ">>> 查询到客户信息: index[%d], isEnd[%c], " \
            ...
            "机构标志[%" __SPK_FMT_HH__ "u], 投资者分类[%" __SPK_FMT_HH__ "u]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N', pCust->custId,
            pCust->custType, pCust->status, pCust->riskLevel,
            pCust->institutionFlag, pCust->investorClass);
}
...
OesClientMySpi::OnQueryCrdCreditAsset(
    ...
    fprintf(stdout, ">>> 查询到信用资产信息: index[%d], isEnd[%c], " \
            ...
            "修正资产金额[%" __SPK_FMT_LL__ "d], " \
            "可转出资产价值[%" __SPK_FMT_LL__ "d], " \
            "标的证券市值[%" __SPK_FMT_LL__ "d]\n",
            pCursor->seqNo, pCursor->isEnd ? 'Y' : 'N',
            ...
            pCreditAsset->correctAssetValue,
            pCreditAsset->trsfOutAbleAssetValue,
            pCreditAsset->underlyingMarketCap);
}
