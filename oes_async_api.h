 ...
 *              - 删除 '期权账户结算单确认接口 (OesAsyncApi_SendOptSettlementConfirmReq)' 中的无效参数
 ...
 *          - 增加查询接口的包裹函数 (基于异步API内置的查询通道执行)
 *              - OesAsyncApi_QueryCrdUnderlyingInfo, 查询客户的融资融券可充抵保证金证券及融资融券标的信息
 ...
 *          - 增加辅助的异步API配置项和配置接口, 以支持启动独立的连接管理线程来执行连接和OnConnect回调处理
 ...
 *              - 重命名并调整字段取值含义: 允许客户端修改密码的开始时间 (changePwdLimitTime) 字段 改为
 *                禁止客户端修改密码的结束时间 (forbidChangePwdEndTime) 字段, 旧字段保持兼容
...
/** 默认的异步API配置区段名称 */
#define OESAPI_CFG_DEFAULT_SECTION_ASYNC_API    "async_api"
/** 默认的CPU亲和性配置区段名称 */
...
#define OESAPI_CFG_DEFAULT_KEY_CPUSET_CONNECT                           \
        "oesapi_connect"
...
#define DEFAULT_OESAPI_ASYNC_CONTEXT_PARAMS                             \
        -1, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE, {0}
/* -------------------------           */
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL    OesAsyncApi_SetOnDisconnect(
                OesAsyncApiChannelT *pAsyncChannel,
                F_OESAPI_ASYNC_ON_DISCONNECT_T fnOnDisconnect,
                void *pOnDisconnectParams);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL    OesAsyncApi_SetOnConnectFailed(
                OesAsyncApiChannelT *pAsyncChannel,
                F_OESAPI_ASYNC_ON_DISCONNECT_T fnOnConnectFailed,
 ...
 */
F_OESAPI_ASYNC_ON_DISCONNECT_T
        OesAsyncApi_GetOnConnectFailed(
                OesAsyncApiChannelT *pAsyncChannel);
 ...
 *
 * - @note 批量委托接口支持的委托数量限制:
 *   - 批量委托接口最大允许传入的委托数量上限为: 10000 笔委托 (@see OES_MAX_BATCH_ORDERS_UPPER_LIMIT)
 *   - 批量委托接口可一次性原子发送的最大委托数量为: 500 笔委托, 超过该数量将自动拆分为多次提交 (@see OES_MAX_BATCH_ORDERS_SINGLE_COMMIT)
 *
 * @param       pAsyncOrdChannel
 *                              异步API的委托连接通道
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL    OesAsyncApi_LoadCpusetCfg2(
                OesAsyncApiContextT *pAsyncContext,
                const char *pCfgFile,
                const char *pCfgSection,
                const char *pCommunicationCpusetKey,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL    OesAsyncApi_SetConnectThreadCpusetCfg(
                OesAsyncApiContextT *pAsyncContext,
...
/**
 * 设置是否启动独立的连接管理线程来执行连接和OnConnect回调处理
 *
 * @param   pAsyncContext       异步API的运行时环境指针
 * @param   isAsyncConnectAble  是否启动独立的连接管理线程来执行连接和OnConnect回调处理
 *                              - TRUE: 创建单独的连接管理线程
 *                              - FALSE: 不启动单独的连接管理线程, 直接在通信线程下执行回调处理
 * @return  TRUE 成功; FALSE 失败
 */
BOOL    OesAsyncApi_SetAsyncConnectAble(
                OesAsyncApiContextT *pAsyncContext,
 ...
 * @return  是否在启动前预创建并校验所有的连接
 */
BOOL    OesAsyncApi_IsAsyncConnectAble(
                OesAsyncApiContextT *pAsyncContext);
 ...
 * 返回内置的查询通道是否已连接就绪
 *
 * @param   pAsyncChannel       异步API的连接通道信息 (委托通道或回报通道均可)
 * @return  TRUE 已连接就绪; FALSE 尚未连接就绪
 */
BOOL    OesAsyncApi_IsBuiltinQueryChannelConnected(
                OesAsyncApiChannelT *pAsyncChannel);
...
/**
 * 查询两地交易时对端结点的资金资产信息 (基于异步API内置的查询通道执行)
 *
 * @note        与同步API接口的不同处:
 *              - 支持自动重建连接
 *              - 线程安全 (内置了加锁处理)
 ...
 * @retval      <0              查询失败 (负的错误号)
 *
 * @see         OesApi_QueryColocationPeerCashAsset
 * @see         OesCashAssetItemT
 */
int32   OesAsyncApi_QueryColocationPeerCashAsset(
                OesAsyncApiChannelT *pAsyncChannel,
                const char *pCashAcctId,
...
/**
 * 查询客户的融资融券可充抵保证金证券及融资融券标的信息 (基于异步API内置的查询通道执行)
 *
 * @note    与同步API接口的不同处:
 *          - 支持自动重建连接
 *          - 线程安全 (内置了加锁处理)
 ...
 * @retval  <0                  失败 (负的错误号)
 *
 * @see     OesApi_QueryCrdUnderlyingInfo
 * @see     OesCrdUnderlyingInfoItemT
 */
int32   OesAsyncApi_QueryCrdUnderlyingInfo(
                OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @retval  <0                  失败 (负的错误号)
 *
 * @see     OesApi_QueryCrdHolding
 * @see     OesStkHoldingItemT
 */
int32   OesAsyncApi_QueryCrdHolding(
                OesAsyncApiChannelT *pAsyncChannel,
                const OesQryStkHoldingFilterT *pQryFilter,
                F_OESAPI_ON_QRY_MSG_T fnQryMsgCallback,
                void *pCallbackParams);
 ...
 * @retval      <0              查询失败 (负的错误号)
 *
 * @see         OesApi_QuerySingleCrdHolding
 * @see         OesStkHoldingItemT
 */
int32   OesAsyncApi_QuerySingleCrdHolding(
                OesAsyncApiChannelT *pAsyncChannel,
                const char *pInvAcctId,
                const char *pSecurityId,
                OesStkHoldingItemT *pOutHoldingItem);
...
/**
 * 查询融资融券业务最大可取资金 (基于异步API内置的查询通道执行)
 *
 * @note    与同步API接口的不同处:
 *          - 支持自动重建连接
 *          - 线程安全 (内置了加锁处理)
 ...
 * @retval      <0              查询失败 (负的错误号)
 *
 * @see     OesApi_GetCrdDrawableBalance
 */
int64   OesAsyncApi_GetCrdDrawableBalance(
                OesAsyncApiChannelT *pAsyncChannel,
                OesCrdDrawableBalanceItemT *pOutDrawableBalanceItem);

/**
 * 查询融资融券担保品可转出的最大数量 (基于异步API内置的查询通道执行)
 *
 * @note    与同步API接口的不同处:
 *          - 支持自动重建连接
 *          - 线程安全 (内置了加锁处理)
 ...
 * @retval      <0              失败 (负的错误号)
 *
 * @see     OesApi_GetCrdCollateralTransferOutMaxQty
 */
int64   OesAsyncApi_GetCrdCollateralTransferOutMaxQty(
                OesAsyncApiChannelT *pAsyncChannel,
                const char *pSecurityId,
                uint8 mktId,
